...
const ShareIcon:React.FC<IconProps> = ({className}) => {
    return (
        <svg aria-label="Share" role="img" viewBox="0 0 18 18"
             ...
             w-[var(--x-width)] h-[var(--x-height)] stroke-2 fill-none stroke-current flex-shrink-0 
             relative block text-[var(--barcelona-charcoal-icon)] [stroke-linejoin:round] 
             [forced-color-adjust:auto] list-none select-none cursor-pointer [-webkit-tap-highlight-color:transparent]`}
             style={{
                    '--x-fill': 'currentColor',
                    '--x-height': '18px',
                    '--x-width': '18px',
