...
const ReplyIcon:React.FC<IconProps> = ({className}) => {
    return (
        <svg aria-label="Reply" role="img" viewBox="0 0 18 18"
             style={{
                    '--x-fill': 'currentColor',
                    '--x-height': '18px',
                    '--x-width': '18px',
             } as React.CSSProperties}
             className={className ? className : `w-[var(--x-width)] h-[var(--x-height)] stroke-2 fill-none flex-shrink-0 
             stroke-current relative block text-[var(--barcelona-charcoal-icon)] [stroke-linejoin:round] [forced-color-adjust:auto]`}
             ><title>Reply</title>
            <path
                ...
                strokeLinejoin="round" strokeWidth="1.25"></path>
        </svg>
    );
};
