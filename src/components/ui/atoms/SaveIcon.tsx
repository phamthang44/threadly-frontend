...
// Mở rộng interface để thêm prop 'filled' (tuỳ chọn)
interface SaveIconProps extends IconProps {
    filled?: boolean;
}

const SaveIcon: React.FC<SaveIconProps> = ({ className, filled = false }: SaveIconProps) => {
    return (
        <svg
        ...
        >
            <title>{filled ? "Unsave" : "Save"}</title>
            <path
