import React from "react";
import { IconProps } from "@/components/ui/atoms/IconProps";
const LikeIconFilled:React.FC<IconProps> = ({className}) => {
    return (
        <svg aria-label="Unlike" role="img" viewBox="0 0 18 18"
             className={className ? className : `
             w-[var(--x-width)] h-[var(--x-height)] fill-[var(--x-fill)] 
             [stroke:currentColor] flex-shrink-0 relative block [stroke-width:1.75] text-[var(--barcelona-notification-badge-background)]
             [stroke-linejoin:round] [forced-color-adjust:auto] list-none select-none cursor-pointer [-webkit-tap-highlight-color:transparent]
             `}
             style={{
                    '--x-fill': 'currentColor',
                    '--x-height': '19px',
                    '--x-width': '18.75px',
             } as React.CSSProperties}><title>Unlike</title>
            <path
                d="M1.34375 7.03125L1.34375 7.04043C1.34374 7.54211 1.34372 8.26295 1.6611 9.15585C1.9795 10.0516 2.60026 11.0779 3.77681 12.2544C5.59273 14.0704 7.58105 15.5215 8.33387 16.0497C8.73525 16.3313 9.26573 16.3313 9.66705 16.0496C10.4197 15.5213 12.4074 14.0703 14.2232 12.2544C15.3997 11.0779 16.0205 10.0516 16.3389 9.15585C16.6563 8.26296 16.6563 7.54211 16.6562 7.04043V7.03125C16.6562 4.73466 15.0849 2.75 12.6562 2.75C11.5214 2.75 10.6433 3.28244 9.99228 3.95476C9.59009 4.37012 9.26356 4.8491 9 5.31533C8.73645 4.8491 8.40991 4.37012 8.00772 3.95476C7.35672 3.28244 6.47861 2.75 5.34375 2.75C2.9151 2.75 1.34375 4.73466 1.34375 7.03125Z"></path>
        </svg>
    );
}

export default LikeIconFilled;